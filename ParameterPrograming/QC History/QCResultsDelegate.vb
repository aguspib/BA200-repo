...
#Region "Public Methods"
        ...
        '''              SA 04/06/2012 - Added parameter for AnalyzerID
        '''              SA 30/01/2015 - BA-1098 ==> When Calculation Mode changes from STATISTICAL to MANUAL, the Results used for the Statistical Mean
        '''                                          of linked Controls/Lots that have been already cumulated (they are included in a Closed RunsGroupNumber) 
        '''                                          have to be deleted instead of unmarked 
        ''' </remarks>
                    ...
                    If (Not dbConnection Is Nothing) Then
                        ...
                        If (Not myGlobalDataTO.HasError) Then
                            If (pCalculationMode = "MANUAL") Then
                                ...
                                Dim myRunsGroupsDelegate As New RunsGroupsDelegate

                                'BA-1098
                                '1.4A Get all linked Controls/Lots for which Results have been already cumulated. For these Controls/Lots, the RunsGroupNumber
                                '     of the Statistical Results used for the Mean is CLOSED, and then the existing Results have to be deleted instead of unmarked
                                myGlobalDataTO = myRunsGroupsDelegate.ReadClosedRunsGroupsWithOpenResults(dbConnection, pQCTestSampleID, pAnalyzerID)
                                If (Not myGlobalDataTO.HasError AndAlso Not myGlobalDataTO.SetDatos Is Nothing) Then
                                    Dim myClosedRunsGroupDS As RunsGroupsDS = DirectCast(myGlobalDataTO.SetDatos, RunsGroupsDS)
                                    For Each row As RunsGroupsDS.tqcRunsGroupsRow In myClosedRunsGroupDS.tqcRunsGroups.Rows
                                        myGlobalDataTO = mytqcResultsDAO.DeleteStatisticResultsNEW(dbConnection, pQCTestSampleID, row.QCControlLotID, pAnalyzerID, row.RunsGroupNumber)
                                        If (myGlobalDataTO.HasError) Then Exit For
                                    Next
                                End If

                                '1.4B Unmark the rest of Statistical Results (those belonging to non-cumulated Controls/Lots)
                                If (Not myGlobalDataTO.HasError) Then
