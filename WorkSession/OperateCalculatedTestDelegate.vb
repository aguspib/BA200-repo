...
'BA-2153: Class has been moved from Calculations Namespace to BL Namespace to allow call function ExecuteCalculatedTest from function
'         SaveOffSystemResults in ResultsDelegate (because from v3.2.0 Calculated Tests can include OFFS Tests in their Formula)
...
#Region "Public methods"
        ...
        '''              SA 14/01/2015 - BA-2153 ==> If at least one of the Tests (whatever type) included in the Formula of the Calculated Test has not a 
        '''                                          valid and accepted Result, if the current status of the Calculated Test is CLOSED, it has to be changed 
        '''                                          to PENDING, due to the previous Result has been deleted.
        ''' </remarks> 
                                ...
                                If (requiredOrderCalculatedTests.vwksOrderCalculatedTests.Rows.Count > 0) Then
                                    ...
                                    For Each orderCalculatedTestsRow1 As ViewOrderCalculatedTestsDS.vwksOrderCalculatedTestsRow In requiredOrderCalculatedTests.vwksOrderCalculatedTests.Rows
                                        ...
                                        If (Not resultData.HasError AndAlso Not resultData.SetDatos Is Nothing) Then
                                            ...
                                            If (Not resultData.HasError AndAlso allComponentTestsHaveResults) Then
                                                ...
                                                If (Not resultData.HasError AndAlso Not resultData.SetDatos Is Nothing) Then
                                                    ...
                                                    If (Not resultData.HasError AndAlso Not resultData.SetDatos Is Nothing) Then
                                                        ...
                                                        resultData = myOrdersDelegate.SetNewOrderToExportValue(dbConnection, , resultRow.OrderTestID)
                                                        If (resultData.HasError) Then Exit For
                                            ...
                                            Else
                                                If (Not resultData.HasError) Then
                                                    'BA-2153: If at least one of the Tests (whatever type) included in the Formula of the Calculated Test has not a valid and
                                                    '         accepted Result, if the current status of the Calculated Test is CLOSED, it has to be changed to PENDING, due to 
                                                    '         the previous Result has been deleted
                                                    If (orderCalculatedTestsRow1.CalcOrderTestStatus = "CLOSED") Then
                                                        resultData = myOrderTestDelegate.UpdateStatusByOrderTestID(dbConnection, orderCalculatedTestsRow1.CalcOrderTestID, "PENDING")
                                                        If (resultData.HasError) Then Exit For
                                                    End If
                                                Else
                ...
                If (minimunValue.HasValue AndAlso maximunValue.HasValue) Then
                    If (minimunValue <> -1 AndAlso maximunValue <> -1) Then
                        ...
                        If (pResultRow.CONC_Value < minimunValue.Value) Then
                            AddResultAlarm(myResultAlarmsDS, pResultRow.OrderTestID, pResultRow.RerunNumber, pResultRow.MultiPointNumber, _
                                           Alarms.CONC_REMARK7.ToString)

                        ElseIf (pResultRow.CONC_Value > maximunValue.Value) Then
                            AddResultAlarm(myResultAlarmsDS, pResultRow.OrderTestID, pResultRow.RerunNumber, pResultRow.MultiPointNumber, _
                                           Alarms.CONC_REMARK8.ToString)
                        End If
                    End If
                End If
