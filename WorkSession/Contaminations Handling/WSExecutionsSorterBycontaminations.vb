...
#Region "Private methods"
            ...
            If ExecutionsSampleTypes.Contains(sortedSampleType) OrElse _
               (ExecutionsSampleClasses.Count = 1 AndAlso ExecutionsSampleClasses.Contains("BLANK")) Then
                ...
                If sampleClass = "PATIENT" OrElse sampleClass = "CTRL" Then 'Apply OrderBy sample type order {"SER", "URI", "PLM", "WBL", "CSF"} (case one PATIENT with several sample types) + execution type
                    ...
                    standardExecutionTypeOrderTests = GetStandardExecutionTypeOrderTestsForPatients(sampleClass, Stat, sampleType, ID)

                Else
                    allTestTypeOrderTests = GetOrderTestsByReadingCycle(sampleClass, Stat, ID)
                    standardExecutionTypeOrderTests = GetStandardExecutionTypeOrderTestsForBlanksAndCallibrators(sampleClass, Stat, ID)
    ...
    Private Function GetStandardExecutionTypeOrderTestsForBlanksAndCallibrators(ByVal sampleClass As String, ByVal Stat As Boolean, ByVal ID As Integer) As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow)
    ...
    Private Function GetStandardExecutionTypeOrderTestsForPatients(ByVal sampleClass As String, ByVal Stat As Boolean, ByVal sampleType As String, ByVal ID As Integer) As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow)
        Dim standardExecutionTypeOrderTests As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow)
        ...
        For Each sortedSampleType In allPossibleSampleTypes
            ...
            If ExecutionsSampleTypes.Contains(sortedSampleType) OrElse _
               (ExecutionsSampleClasses.Count = 1 AndAlso ExecutionsSampleClasses.Contains("BLANK")) Then
                ...
                Dim standardExecutionTypeOrderTests As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow) = Nothing  'Only STD test order tests
                ...
                Else 'Do not apply OrderBy & do not take care about sample type order inside the same SAMPLE
                    AllTestTypeOrderTests = GetOrderTestsByByReadingCycle2(sampleClass, Stat, ID)
                    standardExecutionTypeOrderTests = GetStandardExecutionTypeOrderTestsForBlanksAndCallibrators(sampleClass, Stat, ID)
                End If

                returnDS = SortByElementGroupContamination(sampleClass, Stat, ID, standardExecutionTypeOrderTests, previousElementLastReagentID, previousElementLastMaxReplicates, PreviousReagentsIDList, dbConnection, returnDS, AllTestTypeOrderTests, previousOrderTestMaxReplicatesList)
    ...
    Private Function SortByElementGroupContamination(ByVal sampleClass As String, ByVal Stat As Boolean, ByVal ID As Integer, ByVal standardExecutionTypeOrderTests As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow), ByRef previousElementLastReagentID As Integer, ByRef previousElementLastMaxReplicates As Integer, ByVal PreviousReagentsIDList As List(Of Integer), ByVal dbConnection As SqlConnection, ByVal returnDS As ExecutionsDS, ByVal AllTestTypeOrderTests As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow), ByVal previousOrderTestMaxReplicatesList As List(Of Integer)) As ExecutionsDS
        ...
        If standardExecutionTypeOrderTests.Count > 0 Then
            ...
            If previousElementLastReagentID <> -1 Then  '-1 are blanks, so they do not contaminate.
                ...
                If pendingOrderTestInNewElement.Any Then
                    ...
                    If existContamination.Any Then
                        ...
                        orderContaminationNumber = 1 + ExecutionsDelegate.GetContaminationNumber(contaminationsDataDS, standardExecutionTypeOrderTests, highContaminationPersitance)
                                ...
                                If newPendingOrderTestMaxReplicates < highContaminationPersitance Then
                                    For i = 1 To highContaminationPersitance - 1
                                        ...
                                        If auxI <= pendingOrderTestInNewElement.Count - 1 Then 'Avoid overflow
                                            existContamination = (From wse In contaminationsDataDS.tparContaminations _
                                                Where wse.ReagentContaminatorID = PreviousReagentsIDList(PreviousReagentsIDList.Count - 1) AndAlso
                                                wse.ReagentContaminatedID = pendingOrderTestInNewElement.ElementAt(auxI).ReagentID AndAlso
            ...
            ManageContaminations(dbConnection, returnDS, standardExecutionTypeOrderTests, AllTestTypeOrderTests, orderContaminationNumber, PreviousReagentsIDList, previousOrderTestMaxReplicatesList)
    ...
    Private Function GetOrderTestsByByReadingCycle2(ByVal sampleClass As String, ByVal Stat As Boolean, ByVal ID As Integer) As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow)
        Dim AllTestTypeOrderTests As IEnumerable(Of ExecutionsDS.twksWSExecutionsRow)
